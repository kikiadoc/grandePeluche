<script>
	...
	function myWsCallback(m) {
		if (m.op == "uchronieIntroSynch") {
			...
			if (idx >= phrases.length)
				newInfoPopup(
					"Merci!! Le Message des Quatre a été totalement matérialisé",
	...
	// Gestion de l'épique
	////////////////////////////////////////////
	let epiqStep1 = loadIt("P200_epiqStep1", false);
	$: storeIt("P200_epiqStep1",epiqStep1);
	let epiqStep2 = loadIt("P200_epiqStep2", false);
	$: storeIt("P200_epiqStep2",epiqStep2);
...
<h3>La transition Magique,<br/>Décryptage du Message des Quatre</h3>
<div>Progression du challenge:{#if idx<0}non démarré{:else}{idx}/{phrases.length}{/if}</div>
<div class="phrases scrollBarY" bind:this={scrollPhrases}>
	{#if idx < 0}
...
<div class="gpMessage scrollBarY" >Grande Peluche: {gpMessage}</div>
{#if resultats==null}
	<div class="toBeDiscovered scrollBarY">
		{#if idx < 0}
...
<!-- force du spacing apres le clavier pour eviter les misclick -->
<div>&nbsp;</div>
<div>&nbsp;</div>
<div>&nbsp;</div>
<div>&nbsp;</div>
