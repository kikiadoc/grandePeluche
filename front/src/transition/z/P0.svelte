...
<div style="font-size: 0.7em">
	<div class="adminCadre">
		...
		<input type="button" value="infoGlobalDiscord" on:click={async ()=>{let ret = await apiCall('/discord/admGetRaw'); if (ret.status==200) dspObject=ret.o}} />
	</div>
	<div class="adminCadre">
		...
		<input type="button" value="ClearDiscord/ff14Id" on:click={clearFf14IdDiscord}>
	</div>
	...
	<div class="adminCadre">
		{#if admDiscordActions!=null}
			...
			<div class="adminCadre">
				...
				<input type="button" value="hegemonie" on:click={() => { newChan="hegemonie"} } />
					...
					<div>
						...
						{#each dspPseudo as p}
							<tr>
								...
								<td ><input type="button" value="discord" on:click={async ()=> {let ret = await apiCall('/discord/admGetByFf14Id/'+p[1].ff14Id); if (ret.status==200) dspObject=ret.o } } /></td>
							</tr>
						{/each}
						</table>
					</div>
	...
	{#if dspObject}
		<div class="popupCadre papier">
			<div class="close" on:click={()=>dspObject=null} on:keypress={null} role="button" tabindex=0>X</div>
			<div class="popupZone">
				<div class="popupContent">
					<pre>{JSON.stringify(dspObject,null,2)}</pre>
				</div>
			</div>
		</div>
	{/if}
